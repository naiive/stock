#!/usr/bin/env python3
# -*- coding: utf-8 -*-

"""
============================================================
WaveTrend (LazyBear) 指标 - Python 完整实现

功能:
1. 计算 wt1 / wt2 / wt_diff
2. 识别金叉 / 死叉信号
3. 参数集中配置（类似 TradingView input）
4. 输出每日红绿点信号

作者: 你自己（基于 LazyBear 思路）
============================================================
"""

import pandas as pd
import numpy as np
from api.stock_query import stock_zh_a_daily_mysql


# ------------------------------------------------------------
# Pandas 显示设置（调试用）
# ------------------------------------------------------------
pd.set_option('display.max_rows', None)
pd.set_option('display.max_columns', None)
pd.set_option('display.width', None)


# ============================================================
# WaveTrend 指标计算函数（核心）
# ============================================================
def wavetrend(df, config):
    """
    WaveTrend (LazyBear) 指标计算

    参数说明（完全对应 TradingView input）:
    ------------------------------------------------
    n1      : Channel Length
    n2      : Average Length
    ma_len  : wt2 平滑周期（LazyBear 固定为 4）
    ob1     : 超买线 1（画线 / 过滤用）
    ob2     : 超买线 2
    os1     : 超卖线 1
    os2     : 超卖线 2

    返回:
    ------------------------------------------------
    在 df 中新增以下列:
    wt1        主线（tci）
    wt2        信号线
    wt_diff    动能柱（wt1 - wt2）
    wt_signal  信号:
                1  -> 绿色点（金叉）
               -1  -> 红色点（死叉）
                0  -> 无信号
    """

    # ---------- 读取参数 ----------
    n1 = config.get('n1', 10)
    n2 = config.get('n2', 21)
    ma_len = config.get('ma_len', 4)

    # ---------- HLC3 (TradingView: hlc3) ----------
    # 用于平滑价格，减少单一 close 噪声
    ap = (df['high'] + df['low'] + df['close']) / 3

    # ---------- ESA (EMA of price) ----------
    # 类似“价格基准线”
    esa = ap.ewm(span=n1, adjust=False).mean()

    # ---------- D (价格偏离的 EMA) ----------
    # 衡量波动强度，避免除以 0
    d = (ap - esa).abs().ewm(span=n1, adjust=False).mean()

    # ---------- CI (Composite Index) ----------
    # 核心震荡源
    ci = (ap - esa) / (0.015 * d)

    # ---------- WT1 (主线) ----------
    wt1 = ci.ewm(span=n2, adjust=False).mean()

    # ---------- WT2 (信号线) ----------
    wt2 = wt1.rolling(window=ma_len).mean()

    # ---------- WT 差值（蓝色动能柱） ----------
    wt_diff = wt1 - wt2

    # ---------- 交叉判断 ----------
    # 金叉：wt1 从下向上穿过 wt2
    cross_up = (wt1.shift(1) < wt2.shift(1)) & (wt1 > wt2)

    # 死叉：wt1 从上向下穿过 wt2
    cross_down = (wt1.shift(1) > wt2.shift(1)) & (wt1 < wt2)

    # ---------- 信号编码 ----------
    signal = np.where(
        cross_up, 1,
        np.where(cross_down, -1, 0)
    )

    # ---------- 写回 DataFrame ----------
    df['wt1'] = wt1
    df['wt2'] = wt2
    df['wt_diff'] = wt_diff
    df['wt_signal'] = signal

    return df


# ============================================================
# 主程序入口
# ============================================================
def main():
    # --------------------------------------------------------
    # 1. 读取日线数据
    # --------------------------------------------------------
    df = stock_zh_a_daily_mysql(
        symbol='600588',
        start_date='20240101',
        end_date='20251231',
        adjust='qfq'
    )

    if df is None or df.empty:
        print("数据为空")
        return

    # 字段统一小写，避免不同数据源不一致
    df.columns = [c.lower() for c in df.columns]

    # --------------------------------------------------------
    # 2. WaveTrend 参数配置区（像 TradingView input）
    # --------------------------------------------------------
    wt_config = {
        "n1": 10,     # Channel Length
        "n2": 21,     # Average Length
        "ma_len": 4,  # wt2 平滑周期
        "ob1": 60,    # 超买线 1
        "ob2": 53,
        "os1": -60,   # 超卖线 1
        "os2": -53,
    }

    # --------------------------------------------------------
    # 3. 计算 WaveTrend
    # --------------------------------------------------------
    df = wavetrend(df, wt_config)

    # --------------------------------------------------------
    # 4. 只保留有红绿点的日期
    # --------------------------------------------------------
    signal_df = df[df['wt_signal'] != 0][
        ['date', 'close', 'wt1', 'wt2', 'wt_diff', 'wt_signal']
    ].copy()

    # --------------------------------------------------------
    # 5. 显示层：统一保留两位小数
    # --------------------------------------------------------
    for col in ['close', 'wt1', 'wt2', 'wt_diff']:
        signal_df[col] = signal_df[col].round(2)

    # --------------------------------------------------------
    # 6. 信号可读化
    # --------------------------------------------------------
    signal_df['signal_text'] = signal_df['wt_signal'].map({
        1: '🟢 看多（绿色点）',
       -1: '🔴 看空（红色点）'
    })

    # --------------------------------------------------------
    # 7. 输出结果
    # --------------------------------------------------------
    print(signal_df)


# ============================================================
# 程序入口
# ============================================================
if __name__ == "__main__":
    main()
