...
"""
# ============================================================
# 策略 Squeeze Momentum Indicator [LazyBear]
# ============================================================
"""
...
def tv_linreg(
    ...
    """
    作用：这是一个 线性回归 (Linear Regression) 的辅助函数，用于拟合一段时间序列的趋势
    过程：
        1.构建时间向量 x = [0,1,...,length-1]
        2.用最小二乘法拟合直线 y = m*x + b
        3.返回拟合直线在最新时间点的值 m*(length-1) + b
    目的：平滑处理Squeeze Momentum中的“动能”，得到当前柱状趋势的线性估计

    :param y: 价格或中间指标序列（pandas Series）
    :param length: 回归长度
    :return: 拟合直线在最新时间点的值 m*(length-1) + b
    """
    if pd.isna(y).any():
        return np.nan
    x = np.arange(length)
...
def true_range(df: pd.DataFrame)-> pd.DataFrame:
    """
    作用：计算 真实波幅 (True Range, TR)，这是ATR、波动性指标的基础
    公式：TR = max( high - low,
                   abs(high - previous_close),
                   abs(low - previous_close)
                )
    目的：衡量单根K线的实际波动幅度，用于计算Squeeze Momentum的通道宽度
    """
